"""
Test: Servo eje Y (GP17) de forma aislada.
...
def test_barrido():
    """Barrido completo del eje Y con lectura de voltaje."""
    print("=== TEST EJE Y - Barrido ===")
    step_ms = MAX_TRAVEL_MS // SCAN_STEPS

    print("Ir a extremo (CCW %dms)..." % MAX_TRAVEL_MS)
    servo_mv(srv_y, -1, MAX_TRAVEL_MS)
    ...
    if diff != 0:
        ...
        servo_mv(srv_y, d, abs(diff) * step_ms)
